    ...
    def fit(self, 
        ...
        for iteration in range(max_iter):
            ...
            if verbose and iteration % 100 == 0:
                ...
                print(f"Max Joint error idx: {joint_error.argmax().item()}, corresponding joint: {list(self.mapping_table.keys())[joint_error.argmax().item()]}")
                ...
                print(f"Max End effectors error idx: {end_effectors_error.argmax().item()}, corresponding end effector: {end_effectors[end_effectors_error.argmax().item()]}")
...
if __name__ == "__main__":
    device = 'cuda'
    smplh_model_path = "/home/yifei/dataset/smplh/male/model.npz"
    dmpls_model_path = "/home/yifei/dataset/dmpls/male/model.npz"
    urdf_path = "/home/yifei/dataset/unitree_ros/robots/h1_2_description/h1_2.urdf"
    fitter = SMPLHFitter(
        # smplh_model_path="/home/yyf/RobotCodespace/amass/support_data/body_models/smplh/male/model.npz", dmpls_model_path="/home/yyf/RobotCodespace/amass/support_data/body_models/dmpls/male/model.npz",
        # urdf_path="/home/rl/TurinHumanoid/TurinHumanoidV2/robot_model/full_dof/urdf/full_dof_al_v2.urdf",
        # urdf_path="/home/yyf/codespace/h1_2_description/h1_2.urdf",
        smplh_model_path=smplh_model_path,
        dmpls_model_path=dmpls_model_path,
        urdf_path=urdf_path,
        device=device)

    # print(fitter.smplh_body_names)
    # print(fitter.robot)
    ...
    t_pose_action[dof_names.index("right_shoulder_roll_joint")] = -np.pi/2
    t_pose_action[dof_names.index("left_shoulder_roll_joint")] = np.pi/2
    t_pose_action[dof_names.index("right_elbow_joint")] = np.pi/2
    t_pose_action[dof_names.index("left_elbow_joint")] = np.pi/2
    # offset_map = {
    #         # "Link_knee_l_pitch": torch.Tensor([0.08, 0, 0.0]).to(device),
    #         # "Link_knee_r_pitch": torch.Tensor([0.08, 0, 0.0]).to(device),
    #         # 'Link_ankle_l_pitch': torch.Tensor([-0.08, 0, 0]).to(device),
    #         # 'Link_ankle_r_pitch': torch.Tensor([-0.08, 0, 0]).to(device),
    #         'Link_ankle_l_roll': torch.Tensor([0.15, 0, -0.07]).to(device),
    #         'Link_ankle_r_roll': torch.Tensor([0.15, 0, -0.07]).to(device),
    #     }

    sitting_rotation_map = rotation_map.copy()
    sitting_rotation_map['L_Hip'] = [0, 0, np.pi/2]
    sitting_rotation_map['R_Hip'] = [0, 0, -np.pi/2]
    sitting_rotation_map['L_Knee'] = [0, 0, np.pi/2]
    sitting_rotation_map['R_Knee'] = [0, 0, -np.pi/2]
    ...
    # fitter.visualize_all_23_joints()
    # fitter.fit(max_iter=10000, verbose=True, lr=0.01, offset_map=offset_map, rotation_map=rotation_map)
    fitter.fit(max_iter=0, verbose=True, lr=0.01, offset_map=offset_map, rotation_map=rotation_map, t_pose_action=t_pose_action, joint_weights=joint_weights)
    fitter.load_fit_result(path="/home/yifei/codespace/GBC_45/checkpoints/smplh_checkpoint_iter_140000.pt")
    fitter.visualize_open3d()
    #fitter.visualize_open3d_tpose()

    # fitter.save_fit_result(path="/home/yifei/codespace/GBC_45/checkpoints/smplh_checkpoint_iter_140000.pt")
    #fitter.visualize_all_23_joints()
