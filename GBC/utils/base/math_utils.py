...
def euler_to_rot_mat(axis: str, angle: torch.Tensor) -> torch.Tensor:
    ...
    if angle.shape[-1] != num_axes:
        raise ValueError(f"The number of axes ({num_axes}) must match the "
...
def rot_vec_to_mat(rot_vec: torch.Tensor) -> torch.Tensor:
    ...
    I = torch.eye(3, dtype=rot_vec.dtype, device=rot_vec.device).unsqueeze(0).repeat(B, 1, 1)
    R = I + sin_theta.view(B, 1, 1) * K + (1 - cos_theta).view(B, 1, 1) * K_squared
    ...
    if small_theta.any():
        ...
        R_small = I[small_theta] + sin_theta_small.view(-1, 1, 1) * K_small + one_minus_cos_theta_small.view(-1, 1, 1) * K_squared_small
...
def angle_axis_to_ypr(angle_axis: torch.Tensor) -> torch.Tensor:
    ...
    return torch.stack([yaw, pitch, roll], dim=1) # Shape: (N, 3)
@torch.jit.script
...
def symmetry_smplh_pose(pose: torch.Tensor) -> torch.Tensor:
    ...
    # Clone the rotation matrices to prevent in-place modifications during swapping
    mat_opp_clone = mat_opp.clone()
...
def interpolate_angle_axis(angle_axis_data: torch.Tensor, target_fps: int = 1000, source_fps: torch.Tensor = 120) -> torch.Tensor:
    """
    ...
    for i in range(len(t_target) - 1):
        t = (t_target[i] % (t_original[1] - t_original[0])) / (t_original[1] - t_original[0])  # Normalize interpolation ratio
...
def find_longest_cyclic_subsequence(input_tensor: torch.Tensor, max_distance: float=np.inf) -> Tuple[int, int, int, int]:
    ...
    device = input_tensor.device  # Check the device of the input tensor

    # Perform PCA to reduce dimensions to 3 using torch.pca_lowrank
    # _, _, V = torch.pca_lowrank(input_tensor, q=3, center=True)

    # Perform projection while ensuring compatibility with the device
    # reduced_tensor = torch.matmul(input_tensor, V[:, :3].to(device))

    # Convert the reduced tensor to a NumPy array
    # reduced_array = reduced_tensor.cpu().numpy() if device.type == "cuda" else reduced_tensor.numpy()
...
def contact_to_phase(contact: torch.Tensor, threshold: float = 0.5) -> torch.Tensor:
    ...
    edges  = torch.cat([contact.new_tensor([0]), edges,
                        contact.new_tensor([contact.size(0)])])
    edges = edges.long()
    ...
    for start, end in zip(edges[:-1], edges[1:]):
        ...
        if contact[start] == 1:   # stance
            alpha[start:end] = torch.linspace(
                0.0, threshold, length, device=contact.device, dtype=torch.float32)
        else:                     # swing
            alpha[start:end] = torch.linspace(
                threshold, 1.0, length, device=contact.device, dtype=torch.float32)

    return alpha

def filt_feet_contact(actions: torch.Tensor, root_pos: torch.Tensor, root_rot: torch.Tensor, fk: object, foot_names: List[str], threshold: float = 0.1, foot_ground_offset = 0.045, disable_height: bool = False, debug_visualize: bool = False, debug_save_dir: str | None = None, debug_rv: object = None) -> torch.Tensor:
    """
    ...
    for i in range(actions.shape[0]):
        ...
        root_pos_offset = root_pos[i][min_link_height_indices, 2] - min_link_height + foot_ground_offset
        ...
        height_filtered_indices[cond_0, 0] = torch.arange(link_pos.shape[0]).to(link_pos.device)[cond_0]  # row indices for column 0
        height_filtered_indices[cond_1, 1] = torch.arange(link_pos.shape[0]).to(link_pos.device)[cond_1]  # row indices for column 1
        
        # Calculate the average velocity for each column (0 and 1) across all rows
        avg_vel_0 = link_vel[:, 0].mean()  # avg velocity for link_vel[:, 0]
        avg_vel_1 = link_vel[:, 1].mean()  # avg velocity for link_vel[:, 1]

        # Create conditions to filter out velocities below 10% of the average velocity
        ...
        vel_filtered_indices[cond_vel_0, 0] = torch.arange(link_vel.shape[0]).to(link_pos.device)[cond_vel_0]  # row indices for link_vel[:, 0]
        vel_filtered_indices[cond_vel_1, 1] = torch.arange(link_vel.shape[0]).to(link_pos.device)[cond_vel_1]  # row indices for link_vel[:, 1]
        ...
        avg_contact_height = (torch.sum(link_pos[cond_combined_0, 0, 2]) + torch.sum(link_pos[cond_combined_1, 1, 2])) / (torch.sum(cond_combined_0) + torch.sum(cond_combined_1) + 1e-6)
        sigma_contact_height = torch.std(torch.cat([link_pos[cond_combined_0, 0, 2], link_pos[cond_combined_1, 1, 2]], dim=0))
        ...
        if debug_visualize:
            ...
            axs[0].scatter(height_filtered_indices[:, 0].cpu().numpy(), link_pos[height_filtered_indices[:, 0], 0, 2].cpu().numpy(), color='red', label='Filtered Link 0')
            axs[0].scatter(height_filtered_indices[:, 1].cpu().numpy(), link_pos[height_filtered_indices[:, 1], 1, 2].cpu().numpy(), color='green', label='Filtered Link 1')
            ...
            axs[1].scatter(vel_filtered_indices[:, 0].cpu().numpy(), link_vel[vel_filtered_indices[:, 0], 0].cpu().numpy(), color='red', label='Filtered Link 0 Velocity')
            axs[1].scatter(vel_filtered_indices[:, 1].cpu().numpy(), link_vel[vel_filtered_indices[:, 1], 1].cpu().numpy(), color='green', label='Filtered Link 1 Velocity')
            ...
            axs[3].plot(np.sin(contact_to_phase(feet_contact_signal[:, 0], 0.55).cpu().numpy() * 2 * np.pi), label='Link 0 Phase Signal', linestyle='--')
            axs[3].plot(np.sin(contact_to_phase(feet_contact_signal[:, 1]).cpu().numpy() * 2 * np.pi), label='Link 1 Phase Signal', linestyle='--')
...
def simple_moving_average(x: torch.Tensor, window_size: int) -> torch.Tensor:
    ...
    smoothed_reshaped = torch.nn.functional.conv1d(
        x_reshaped, 
        weight, 
        padding=padding, 
        groups=D  
...
def is_foot_parallel_from_rot_matrix(rot_mat_batch: torch.Tensor, tolerance_deg: float = 5.0) -> torch.Tensor:
    ...
    foot_world_normal = torch.matmul(
        rot_mat_batch, 
        foot_local_normal.view(1, 1, 3, 1)
    ).squeeze(-1)
